...
| **B19** | Test fixtures that cannot fail loudly | 176 service-role writes in `supabase/tests/` discard their result, and 54 tests are satisfied by an empty set alone. Both hide the same thing: a fixture that never wrote looks exactly like the refusal the test is asserting. This is the shape that made the realtime failure take a day to find, and the same silent-empty pattern that hid anon's grants on `profiles`. A sweep, not a fix — worth its own pass. |
| **B20** | Five components over 400 lines | `CourseBuilder` 566, `QuizEditor` 533, `UserManager` 495, `SupportInbox` 466, `ContentManager` 463. They are also the least-tested part of the frontend. Splitting them is a real refactor and was deliberately not bundled into the audit. |
| **B21** | 292 inline `style={{}}` props | Concentrated in the authoring and review screens. `src/components/ui/` and the four stylesheets (4,251 lines) already carry the design system, so these are the leftovers that never moved. Cosmetic debt, no behaviour attached. |
| **B22** | Frontend coverage at 74% statements / 77% lines | The gaps are the activity runtimes (flashcards, matching, scenario), the two chart components, `BadgeShelf`, `JourneyMap`, `ProgressRing`, and several `ui/` primitives that are only exercised through the pages using them. Presentational, but the activity runtimes are what a trainee actually touches. |
