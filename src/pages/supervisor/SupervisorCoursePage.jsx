  ...
  return (
    <div className="page-body" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', paddingBottom: '4rem' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
        <button className="btn btn-ghost btn-sm" onClick={() => navigate('/supervisor/courses')}>← Back to Team Courses</button>
      </div>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: '1.5rem', flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1, minWidth: '300px' }}>
            <p style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.6)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '0.4rem' }}>Supervisor Panel • Course Overview</p>
            ...
            <p style={{ fontSize: '0.85rem', color: 'rgba(255,255,255,0.6)', marginTop: '0.5rem' }}>Instructor: <strong>{trainer?.name || 'Unassigned'}</strong></p>
          </div>
          <div style={{ textAlign: 'right', minWidth: '150px' }}>
            <div style={{ fontFamily: 'var(--font-heading)', fontSize: '3.2rem', fontWeight: 700, color: '#fff', lineHeight: 1 }}>{course.progress}%</div>
            <div style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.6)' }}>Team Cohort Progress</div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
              <div className="card no-hover" style={{ padding: 0, overflow: 'hidden', borderRadius: 'var(--r-lg)' }}>
                ...
                <div style={{ padding: '1.5rem' }}>
                  ...
                  <p style={{ color: 'var(--text-2)', fontSize: '0.9rem' }}>This is the active video lecture presented to enrolled trainees. Review the content to check alignment with course objectives.</p>
                </div>
              </div>
            </div>
          )}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1.5rem' }}>
              {!course.materials || course.materials.length === 0 ? (
                ...
                course.materials.map((mat, i) => {
                  ...
                  return (
                    <div key={i} className="card" style={{ display: 'flex', alignItems: 'center', gap: '1rem', cursor: 'pointer' }} onClick={() => alert(`Previewing ${mat.name} as Supervisor...`)}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                ...
                <div className="card no-hover" style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', padding: '1rem' }}>
                  ...
                  {course.activityId && activities[course.activityId] && (
                    <div className="student-row" style={{ cursor: 'pointer', background: 'var(--bg)', border: '1px solid var(--border)' }} onClick={() => alert("Activity Preview (Supervisor Mode)")}>
                  ...
                  {/* Quiz */}
                  <div className="student-row" style={{ cursor: 'pointer', background: 'var(--bg)', border: '1px solid var(--border)' }} onClick={() => alert("Quiz Preview (Supervisor Mode)")}>
          ...
          {activeTab === 'chat' && (
            <div className="card no-hover" style={{ display: 'flex', flexDirection: 'column', height: '600px', padding: 0, overflow: 'hidden' }}>
              <div style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid var(--border)', background: 'var(--surface-alt)' }}>
                <h3 style={{ fontSize: '1.1rem', marginBottom: '0.25rem' }}>💬 Supervisor Board: {course.title}</h3>
                <p style={{ fontSize: '0.8rem', color: 'var(--text-3)' }}>Monitor chat, answer trainee queries, and consult with the instructor.</p>
              </div>
              ...
              <div ref={chatScrollRef} className="chat-messages" style={{ flex: 1, padding: '1.5rem', overflowY: 'auto' }}>
                {messages.length === 0 ? (
                  <div style={{ textAlign: 'center', color: 'var(--text-3)', marginTop: '2rem' }}>
                    <p>No messages yet.</p>
                    <p style={{ fontSize: '0.85rem' }}>Be the first to post an update!</p>
                  </div>
                ) : (
                  messages.map((msg) => {
              ...
              <form onSubmit={handleSendChat} style={{ padding: '1rem', borderTop: '1px solid var(--border)', display: 'flex', gap: '0.5rem', background: 'var(--surface-alt)' }}>
                <input
                  ...
                  placeholder="Type a message as Supervisor..."
                />
